...
class ICNet:
    def __init__(self, sess, input_size, action_num):
        ...
        W1 = tf.Variable(tf.random_uniform([input_size, 20], -0.5, 0.5))
        b1 = tf.Variable(tf.random_uniform([20], -0.5, 0.5))
        l1 = tf.nn.relu(tf.matmul(self.x, W1)+b1)
        W2 = tf.Variable(tf.random_uniform([20, action_num], -0.5, 0.5))
        b2 = tf.Variable(tf.random_uniform([action_num], -0.5, 0.5))
